...
run flux_sp1a 0 30091
run flux_sp1b 0 30092
run flux_sp2  0,1 30093 --num-gpus 2 --ulysses-degree 2 --ring-degree 1
run flux_sp4  0,1,2,3 30094 --num-gpus 4 --ulysses-degree 4 --ring-degree 1
...
SGLANG_SP_TEXT_SHARD_MIN=99999 \
  run flux_sp4_replicate 0,1,2,3 30095 --num-gpus 4 --ulysses-degree 4 --ring-degree 1
...
baseline = load("flux_sp1a")
if baseline is None:
    raise SystemExit("no baseline image")
for label in ("flux_sp1b", "flux_sp2", "flux_sp4", "flux_sp4_replicate"):
    other = load(label)
    if other is None:
